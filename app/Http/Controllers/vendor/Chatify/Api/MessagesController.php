...
class MessagesController extends Controller
{
    ...
    /**
     * Show all users except the authenticated user.
     *
     * @param  Request  $request
     * @return \Illuminate\Http\JsonResponse
     */
        public function showAllUsers(Request $request)
    {
        try {
            ...
            $records = User::where('id', '!=', Auth::id())
                            ->paginate($request->per_page ?? 10); // Default pagination limit to 10
    
            $users = [];
            foreach ($records->items() as $record) {
                $users[] = Chatify::getUserWithAvatar($record);
            }
            ...
            // Log the error message
            \Log::error('Error in ChatifyNewFile@showAllUsers: ' . $e->getMessage());
